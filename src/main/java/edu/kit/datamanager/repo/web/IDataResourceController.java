...
public interface IDataResourceController extends IGenericResourceController<DataResource>, IControllerAuditSupport {
    ...
    @Operation(summary = "Create a new resource.", description = "Create a new resource and return it to the caller in case of success. "
            + "Creating new resources may or may not be restricted to users possessing specific roles.", security = {
                @SecurityRequirement(name = "bearer-jwt")})
    @RequestMapping(value = {"/"}, method = {RequestMethod.POST}, consumes = {"application/json", "application/json+datacite"}
    )
    @ResponseBody
    @Override
